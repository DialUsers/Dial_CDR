...
def get_ols_check_spatial_correlation(south_shape,location_shp_file,year):
    
    #Get the log transform of dependent and independent variables
    X_log = np.log(south_shape['Call_density'])
    Y_log = np.log(south_shape['{}_pop_density'.format(year)])
    ###log transform of Independent and dependent variable#########################
    ...
    if mi_distance.p_norm<0.05:
        ...
        if lms.lme[1]<0.05:
            ...
            south_spat_err_distance=spat_err.summary
              
    return south_ols, a


#Random K-fold
def random_Kfold_results(south_shape,year):
    #Get the log transform of dependent and independent variables
    X = pd.DataFrame(np.log(south_shape['Call_density'])).reset_index(drop=True)
    y = pd.DataFrame(np.log(south_shape['{}_pop_density'.format(year)])).reset_index(drop=True)
    #leave one out 
    loo = LeaveOneOut()
    loo.get_n_splits(X)
...
#spatial K-fold   
def spatial_kfold(south_shape,year,thresh):
    
    data=south_shape[['Call_density','geometry', '{}_pop_density'.format(year)]].reset_index(drop=True)
...
if __name__ == "__main__":
    ...
    shp_south_pcd_year="south_malawi_{}_new".format(year)
    south_shp = dir_shp_write + shp_south_pcd_year +'.shp'
    
    #configuration
    thresh = pysal.min_threshold_dist_from_shapefile(south_shp)
    ...
    malawi_pcd_south=malawi_pcd[malawi_pcd['Zone']=='South']
    write_shape_data_file(dir_shp_write,shp_south_pcd_year,malawi_pcd_south)
    
    central_ols_results,p_value=get_ols_check_spatial_correlation(malawi_pcd_south,south_shp,year)
    
    random_kfold_coef=random_Kfold_results(malawi_pcd_south,year)
    
    randm_spkfold_coef=spatial_kfold(malawi_pcd_south,year,thresh)
