...
#Using ols (pysal) to check spatial correlation
def get_ols_check_spatial_correlation(north_shape,location_shp_file,year):
    
    #Get the log transform of dependent and independent variables
    X_log = np.log(north_shape['Call_density'])
    Y_log = np.log(north_shape['{}_pop_density'.format(year)])
    ...
    if mi_distance.p_norm<0.05:
        ...
        if lms.lme[1]<0.05:
            ...
            north_spat_err_distance=spat_err.summary
...
#Random K-fold
def random_Kfold_results(north_shape,year):
    
    #Get the log transform of dependent and independent variables
    X = pd.DataFrame(np.log(north_shape['Call_density'])).reset_index(drop=True)
    y = pd.DataFrame(np.log(north_shape['{}_pop_density'.format(year)])).reset_index(drop=True)
...
#spatial K-fold   
def spatial_kfold(north_shape,year,thresh):
    
    data=north_shape[['Call_density','geometry', '{}_pop_density'.format(year)]].reset_index(drop=True)
...
if __name__ == "__main__":
    ...
    north_ols_results,p_value=get_ols_check_spatial_correlation(malawi_pcd_north,north_shp,year)
    
    random_kfold_coef=random_Kfold_results(malawi_pcd_north,year)
    
    randm_spkfold_coef=spatial_kfold(malawi_pcd_north,year,thresh)
